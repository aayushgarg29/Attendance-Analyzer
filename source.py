...
class AttendanceCounter:
    def __init__(self, master):
        ...
        # Attendance buttons
        self.present_button = tk.Button(master, text="Mark Present", command=self.mark_present)
        self.present_button.pack(pady=5)

        self.absent_button = tk.Button(master, text="Mark Absent", command=self.mark_absent)
        self.absent_button.pack(pady=5)

        self.visualize_button = tk.Button(master, text="Visualize Attendance", command=self.visualize_attendance)
    ...
    def mark_present(self):
        ...
        if selected_student:
            ...
            messagebox.showinfo("Success", f"{selected_student} marked as Present!")
        else:
    ...
    def mark_absent(self):
        ...
        if selected_student:
            ...
            messagebox.showinfo("Success", f"{selected_student} marked as Absent!")
        else:
            messagebox.showwarning("Input Error", "Please select a student.")

    def visualize_attendance(self):
        # Prepare data for visualization
        attendance_summary = {student: sum(1 for record in records if record == "Present")
                              for student, records in self.attendance.items()}

        # Create a DataFrame
        df = pd.DataFrame(list(attendance_summary.items()), columns=['Student', 'Present Count'])
